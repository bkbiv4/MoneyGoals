...
class CreateAssetVC: UIViewController {
    ...
    var account: Account? {
        didSet {
            setUI()
            print("Setting up")
            brokerText.text = account?.broker?.brokerName
            accountText.text = account?.accountName
            accountNumber.text = account?.accountNumber
            accountBalance.text = convertCurrencyToString(amount: account!.accountBalance)
        }
    }
    ...
    let pickerToolBar : UIToolbar = {
        ...
        let doneButton = UIBarButtonItem(barButtonSystemItem: .done, target: self, action: #selector(saveAccount))
...
//        NotificationCenter.default.addObserver(self, selector: #selector(keyboardWillShow), name: UIResponder.keyboardWillShowNotification, object: nil)
    ...
    @objc func saveNewAcount() {
        ...
        let accountTuple = CoreDataManager.shared.createAccountAsset(accountName: accountName, accountBroker: accountBroker, accountST: "Checkings", accountBalance: accountBalance, accountNumber: accountNumber)
    ...
    @objc func saveAccountChanges() {
        let context = CoreDataManager.shared.persistentContainer.viewContext
        account?.broker?.brokerName = brokerText.text
        account?.accountName = accountText.text
        account?.accountNumber = accountNumber.text
        account?.accountBalance = convertStringToCurrency(input: self.accountBalance.text!)!
        
        do {
            try context.save()
            //If save successful
            dismiss(animated: true, completion: {
                self.delegate?.editAccount(account: self.account!)
            })

        } catch let savErr {
            print("Failed to save the firm", savErr)
        }
    }
...
//        let height = view.bounds.size.height
        ...
        self.view.frame = CGRect(x: 0, y: 150, width: width, height: 300)
    ...
    func setUI() {
        ...
        creditLabel = createLabel(string: "Manual Bank Card", font: UIFont.font18, color: .black, alignment: .center)
        ...
        creditLabel.heightAnchor.constraint(equalToConstant: 30).isActive = true
        ...
        brokerText.heightAnchor.constraint(equalToConstant: 30).isActive = true
        ...
        accountText.heightAnchor.constraint(equalToConstant: 30).isActive = true
        ...
        accountBalance.heightAnchor.constraint(equalToConstant: 30).isActive = true
        ...
        accountNumber.heightAnchor.constraint(equalToConstant: 30).isActive = true
    }
